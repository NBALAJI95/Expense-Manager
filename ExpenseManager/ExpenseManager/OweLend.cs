...
namespace ExpenseManager
{
    public partial class OweLend : Form
    {
        ...
        private void OweLend_Load(object sender, EventArgs e)
        {
        ...
        private void delete_Click(object sender, EventArgs e)
        {
            ...
            cmd.Parameters.AddWithValue("@amt", amt.Text);
            MessageBox.Show("$ " + desc.Text);
            MessageBox.Show("@ " + amt.Text);
            SqlDataReader DR2 = cmd.ExecuteReader();
            while (DR2.Read())
            {
                uname = DR2[0].ToString();
                MessageBox.Show("Income - " + uname.ToString());
                AccountType = DR2[1].ToString();
                MessageBox.Show("Income - " + AccountType.ToString());
            }
            DR2.Close();
            ...
            {
                v = float.Parse(DR2[0].ToString());
                MessageBox.Show("Income B - " + v.ToString());
                v = v - float.Parse(amt.Text);
                MessageBox.Show("Income Sub - " + v.ToString());
            }
            ...
            {
                v = float.Parse(DR2[0].ToString());
                MessageBox.Show("Expense v before - " + v.ToString());
                v = v + float.Parse(amt.Text);
            ...
            MessageBox.Show("Expense v after -" + v.ToString());
            MessageBox.Show("Expense - u - " + uname.ToString());
            MessageBox.Show("Expense - account -" + AccountType.ToString());
            cmd.Parameters.AddWithValue("@user_n", uname);
            cmd.Parameters.AddWithValue("@acct", AccountType);
            cmd.ExecuteNonQuery();
